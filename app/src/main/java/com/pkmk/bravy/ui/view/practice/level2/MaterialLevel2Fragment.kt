...
class MaterialLevel2Fragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        // Observer untuk hasil speech-to-text
        viewModel.speechResult.observe(viewLifecycleOwner) { resultType ->
            ...
            if (isResumed && currentScene?.sceneType == "SPEECH_PRACTICE") {
                // Untuk Level 2, kita asumsikan semua respons suara dianggap "sukses"
                // dan akan memutar video respons yang sama.
                playResponseVideo(currentScene)
            }
        }
    ...
    private fun playResponseVideo(scene: LearningScene?) {
        viewModel.setMicControlsVisibility(false)
        // Mengambil URL dari "success" karena Level 2 hanya punya satu respons
        val responseUrl = scene?.responses?.get("success")
    ...
    companion object {
        ...
        fun newInstance(section: LearningSection) = MaterialLevel2Fragment().apply {
